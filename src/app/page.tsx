...
export default function Home() {
  ...
  const [c, setC] = React.useState(character);
  ...
  return (
    ...
    >
      <div className="w-fit">
        <h1 className="text-6xl font-gothic">
          Chronicle
        </h1>
        <h2
...
function Stats({
  ...
  const getTalisman = (week: number, day: number) => {
    ...
    // console.log("🚀 ~ getTalisman ~ date:", date)
...
function RankWheel({ onSelect }) {
  ...
  const
    numOptions          = Object.keys(ranks).length,
    optionCircumference = (optionSize + optionGap) * numOptions,
    wheelDiameter       = optionCircumference / Math.PI + (borderGap + optionSize + borderWidth) * 2,
    wheelRadius         = wheelDiameter / 2,
    optionRadius        = wheelRadius - borderGap - optionSize / 2 - borderWidth
  ;
